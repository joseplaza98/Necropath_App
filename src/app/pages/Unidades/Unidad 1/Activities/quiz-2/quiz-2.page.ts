...
export class Quiz2Page implements OnInit {
  ...
  loadQuestions() {
    this.http.get<any[]>('/assets/quiz-questions.json').subscribe( //Ruta donde se almacena el json del quiz
      data => {
        console.log('Data cargada:', data); // Verifica la estructura de los datos
        if (Array.isArray(data)) {
          ...
          if (quizData) {
            ...
            console.error('No se encuentran preguntas para quiz 2');
          }
        } else {
  ...
  async saveResults() {
    ...
    if (userId) {
      await this.firestoreService.saveUserScore(userId, "u1_quiz 2", this.correctAnswers, this.totalQuestions); // Cambia el número del test si es necesario
    }
  }
