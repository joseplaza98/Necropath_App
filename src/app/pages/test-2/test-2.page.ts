...
@Component({
  selector: 'app-test-2',
  templateUrl: './test-2.page.html',
  styleUrls: ['./test-2.page.scss'],
})
export class Test2Page implements OnInit {
  ...
  loadQuestions() {
    this.http.get<any[]>('/assets/questions.json').subscribe(
      data => {
        // Filtrar solo las preguntas para el test 2
        this.questions = data.filter(q => q.testNumber === 2)[0]?.questions || [];
  ...
  async submit() {
    ...
    if (userId) {
      await this.firestoreService.saveUserScore(userId, 2, correctAnswers, totalQuestions);
    }
  }
}
